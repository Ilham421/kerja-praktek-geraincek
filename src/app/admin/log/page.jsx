...
export default function LogPage() {
  ...
  const getActionBadgeColor = (action) => {
    switch (action) {
      case "INSERT": return "border-emerald-200 bg-emerald-50";
      case "UPDATE": return "border-amber-200 bg-amber-50";
  ...
  const getTableLabel = (tableName) => {
    switch (tableName) {
      case "service_tickets": return "Servis";
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Catatan semua aktivitas INSERT, UPDATE, DELETE</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Total", value: logs.length, color: "bg-slate-500", icon: "📋" },
          { label: "INSERT", value: logs.filter(l => l.action === "INSERT").length, color: "bg-emerald-500", icon: "➕" },
          { label: "UPDATE", value: logs.filter(l => l.action === "UPDATE").length, color: "bg-amber-500", icon: "✏️" },
          { label: "DELETE", value: logs.filter(l => l.action === "DELETE").length, color: "bg-red-500", icon: "🗑️" },
        ].map((stat) => (
          <div key={stat.label} className="bg-white p-3 sm:p-4 rounded-xl border border-slate-200 hover:shadow-md transition-shadow">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredLogs.length > 0 ? filteredLogs.map((log) => (
                <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-slate-500 whitespace-nowrap">
                    {getTableIcon(log.table_name)} {getTableLabel(log.table_name)}
                  </td>
                ...
                <tr>
                  <td colSpan={5} className="px-4 py-10 text-center text-slate-400">
                    {filter === "all" ? "📭 Belum ada aktivitas tercatat" : `Tidak ada log ${filter}`}
                  </td>
                </tr>
              )}
            </tbody>
          ...
          >
            ...
            <div className="flex items-center justify-between pt-2 border-t border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-slate-600">{log.username || "System"}</span>
              </div>
            </div>
          </div>
        )) : (
      ...
      {/* Empty State untuk Tablet (md) ke atas */}
